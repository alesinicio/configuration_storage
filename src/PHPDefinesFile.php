 ...
 * The data is stored as DEFINEs.
 */
class PHPDefinesFile extends ConfigurationStorageCommon implements ConfigurationStorageInterface {
	private $configurationFilename	= null;
	private $newConfigurationFile	= null;
	private $arrConfiguration		= [];
	private $arrOldConfiguration	= [];
	...
	private function generateNewLine($var) : string {
		if (isset($this->arrConfiguration[$var])) {
			$val = $this->arrConfiguration[$var];
		} else if (isset($this->arrOldConfiguration[$var])) {
			$val = $this->arrOldConfiguration[$var];
		} else {
			throw new Exception('Error retrieving value for `'.$var.'`');
		}

		$val = self::formatValues($val);
		return 'define(\''.$var.'\', '.$val.');'.PHP_EOL;
	}
	/**
